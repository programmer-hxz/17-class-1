namespace WpfApp1.Triton.Common
{
    ...
    public class JsonSettings : NotificationObject, IDisposable
    {
        ...
        protected T GetDefaultValue<T>(Expression<Func<T>> exp)
        {
            ...
            if (((MemberExpression)exp.Body).Member.GetCustomAttributes(typeof(DefaultValueAttribute), true).FirstOrDefault<object>() is DefaultValueAttribute attribute)
            {
        ...
        public static string SettingsPath =>
            Path.Combine(AssemblyPath, "Settings");
    }
}
